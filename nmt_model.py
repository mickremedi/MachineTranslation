...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size: int, hidden_size: int, vocab: Vocab, dropout_rate=0.2):
        """ Init NMT Model.

        @param embed_size: Embedding size (dimensionality)
        @param hidden_size: Hidden Size, the size of hidden states (dimensionality)
        @param vocab: Vocabulary object containing src and tgt languages
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        last_hidden_catted = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        last_cell_catted = torch.cat((last_cell[0], last_cell[1]), dim=1)

        init_decoder_hidden = self.h_projection(last_hidden_catted)
        init_decoder_cell = self.c_projection(last_cell_catted)
    ...
    def step(self, y_bar_t: torch.Tensor,
        ...
        @param y_bar_t: Concatenated Tensor of [Y_t o_prev], with shape (b, e + h). The input for the decoder,
        ...
        @param enc_hiddens_proj: Encoder hidden states Tensor, projected from (h * 2) to h. Tensor is with shape
                                    (b, src_len, h), where b = batch size, src_len = maximum source length, h = hidden
                                    size.
        @param enc_masks: Tensor of sentence masks shape (b, src_len),
                                    where b = batch size, src_len is maximum source length. 

        @returns dec_state (tuple (Tensor, Tensor)): Tuple of tensors both shape (b, h), where b = batch size, h =
                                    hidden size. First tensor is decoder's new hidden state, second tensor is decoder's
                                    new cell.
        @returns combined_output (Tensor): Combined output Tensor at timestep t, shape (b, h), where b = batch size,
                                    h = hidden size.
        ...
        dec_state = self.decoder(y_bar_t, dec_state)
